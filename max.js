...
function pull(prob, pityCounters, pityHit) {
    ...
    */
    ...
    /*
    If n 4*s are pulled, then the next item drawn should be 5*
    */
    if (pityCounters.numFourStars == (pityHit[5] / pityHit[4]) - 1) {
        probabilities[5] += probabilities[4];
        probabilities[4] = 0;
    }
    ...
    if (result == 5) {
        pityCounters[5] = 0;
        pityCounters[4] = 0;
        pityCounters.numFourStars = 0;
    } else if (result == 4) {
        pityCounters[4] = 0;
        pityCounters[5]++;
        pityCounters.numFourStars++;
    } else {
        pityCounters[4]++;
        pityCounters[5]++;
    }
...
// log it
fs.writeFile("Output/charactermax.txt", os, function (err, file) {
...
// log it
fs.writeFile("Output/weaponmax.txt", os, function (err, file) {
    if (err) console.error(err);
});